var Cut2LuaScene = cc.Scene.extend({
	onEnter : function() {
		this._super();
		var size = cc.Director.getInstance().getWinSize();

		var nomeDoPlaneta = cc.LabelTTF.create("LUA", "Impact", 48);
		...
		var clickToPlay = cc.LabelTTF.create("Marcello Abtibol - Level Design\nMarcello Abtibol - Landscape Art", "Arial", 24);
		clickToPlay.setPosition(size.width / 2, size.height / 2);
		this.addChild(clickToPlay, 1);

		var imagemDoPlaneta = cc.Sprite.create(s_lua_cut_image);
		imagemDoPlaneta.setPosition(size.width / 2, size.height / 2);
		this.addChild(imagemDoPlaneta, 0);

	},
	onGo : function() {
		cc.Director.getInstance().replaceScene(cc.TransitionFade.create(1, new Level2LuaScene()));
	}
});
